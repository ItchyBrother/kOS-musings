...
// FIXED: Get Pe coordinates accounting for body rotation
FUNCTION GET_PE_COORDINATES_AT_TIME {
    PARAMETER orbit_patch.
    
    // The orbit patch has a POSITION - that's where it enters Kerbin SOI
    // But we want Pe position, not entry position
    
    // Use the patch's own data about where Pe is
    LOCAL pe_altitude IS orbit_patch:PERIAPSIS.
    LOCAL pe_radius IS KERBIN:RADIUS + pe_altitude.
    
    // Get the Pe position using orbital velocity direction
    // At Pe, velocity is perpendicular to position
    LOCAL pe_eta IS orbit_patch:ETA:PERIAPSIS.
    
    // Actually, let's just try the simplest thing:
    // Don't do ANY rotation adjustment - just read the coordinates at Pe time
    ...
    // Return AS-IS with NO adjustments
    RETURN LEXICON("lat", geo:LAT, "lng", geo:LNG).
}
...
FUNCTION CHECK_ZONE_FEASIBILITY {
    ...
    FROM {LOCAL offset IS 0.} UNTIL offset > period STEP {SET offset TO offset + time_step.} DO {
        ...
        // Match main search dV range and steps
...
FUNCTION MAIN {
    ...
    //PRINT "Checking landing zone feasibility...".
    //PRINT "(Accounting for Kerbin rotation during transit)".
    //PRINT " ".
    ...
    // // DEBUG 
    // PRINT "DIAGNOSTIC: About to start main search".
    // PRINT "  Time now: " + TIME:SECONDS.
    // PRINT "  Base dV: " + ROUND(base_dv, 1).
    // PRINT "  Target Pe: " + target_kerbin_pe.
    // PRINT "  Pe range: 28000 to 40000".
    // PRINT "  Use landing zone: " + use_landing_zone.
    // PRINT "  Target: " + target_lat + ", " + target_lng.
    // PRINT " ".
    ...
    LOCAL last_debug_print IS TIME:SECONDS.  // Add this
    LOCAL period IS SHIP:ORBIT:PERIOD.
    LOCAL time_step IS period / 36.  // Finer resolution: 36 positions

    // //DEBUG
    // LOCAL trajectories_checked IS 0.
    // LOCAL encounters_found IS 0.
    // //DEBUG
    ...
    FROM {LOCAL offset IS 0.} UNTIL offset > period STEP {SET offset TO offset + time_step.} DO {
        ...
        FROM {LOCAL dv_offset IS -60.} UNTIL dv_offset > 120 STEP {SET dv_offset TO dv_offset + 8.} DO {
            ...
            ADD test_node.
            // //DEBUG
            // SET trajectories_checked TO trajectories_checked + 1.
            // //DEBUG
            WAIT 0.1.
            ...
            IF test_node:ORBIT:HASNEXTPATCH AND test_node:ORBIT:NEXTPATCH:BODY = KERBIN {
                ...
                // //DEBUG
                // SET encounters_found TO encounters_found + 1.
                // //DEBUG
                
                // Only consider if Pe is somewhat reasonable (20-50km range)
                IF kerbin_pe > 28000 AND kerbin_pe < 40000 {
                    LOCAL arrival_time IS TIME:SECONDS + test_node:ORBIT:NEXTPATCH:ETA:PERIAPSIS.
                    LOCAL pe_coords IS GET_PE_COORDINATES_AT_TIME(test_node:ORBIT:NEXTPATCH).
                    
                    LOCAL score IS 0.
                    
                    IF use_landing_zone {
                        // Landing zone mode: Heavily weight both Pe accuracy AND landing zone
                        ...
                        // Score: distance in km + Pe error * 50 (makes Pe important too)
                        SET score TO distance_to_target + pe_error * 500.
                        
                    } ELSE {
                        // Basic mode: ONLY Pe accuracy matters
    ...
    // //DEBUG
    // // After the loops, before the "IF NOT found_any":
    // PRINT "Checked " + trajectories_checked + " trajectories".
    // PRINT "Found " + encounters_found + " Kerbin encounters".

    // LOCAL solutions_text IS "ZERO".
    // IF found_any {
    //     SET solutions_text TO "at least one".
    // }
    // PRINT "Found " + solutions_text + " suitable solutions".
    // //DEBUG
    IF NOT found_any {
    PRINT " ".
    ...
    // PRINT " ".
    // PRINT "DEBUG INFO:".
    // PRINT "  Searched " + ROUND(period / time_step, 0) + " time positions".
    // PRINT "  Searched dV range: " + ROUND(base_dv - 60, 1) + " to " + ROUND(base_dv + 120, 1) + " m/s".
    // PRINT "  Required Pe range: 20-28 km".
    // PRINT " ".
    // PRINT "This means NO trajectory in that range gives Kerbin encounter".
    ...
    IF SHIP:ORBIT:HASNEXTPATCH AND SHIP:ORBIT:NEXTPATCH:BODY = KERBIN {
        ...
        LOCAL arrival_time IS TIME:SECONDS + SHIP:ORBIT:NEXTPATCH:ETA:PERIAPSIS.
        LOCAL actual_coords IS GET_PE_COORDINATES_AT_TIME(SHIP:ORBIT:NEXTPATCH).
        
        PRINT "TRAJECTORY STATUS:".
        PRINT "  Kerbin Encounter: Confirmed".
        PRINT "  Kerbin Periapsis: " + ROUND(actual_pe/1000, 1) + " km".
        PRINT "  Landing coords:   " + ROUND(actual_coords["lat"], 1) + "° / " + ROUND(actual_coords["lng"], 1) + "°".
        
        IF use_landing_zone {
            LOCAL dist IS GREAT_CIRCLE_DISTANCE(actual_coords["lat"], actual_coords["lng"], target_lat, target_lng).
            PRINT "  Miss distance:    " + ROUND(dist/1000, 0) + " km from " + zone_name.
            PRINT " ".
            
            IF dist < 150000 {
                PRINT "  ✓ EXCELLENT - On target!".
            } ELSE IF dist < 400000 {
                PRINT "  ✓ GOOD - Small MCC may improve accuracy".
            } ELSE {
                PRINT "  ⚠ MCC recommended to correct landing zone".
            }
        }
        ...
        IF pe_error < pe_tolerance {
            ...
            PRINT "  ⚠ Pe TOO LOW - steep reentry, high G-forces".
            PRINT "    Recommend MCC to raise Pe before entry".
        } ELSE IF actual_pe > 45000 {
            PRINT "  ⚠ Pe TOO HIGH - may skip off atmosphere".
            PRINT "    Recommend MCC to lower Pe before entry".
        } ELSE {
            PRINT "  ⚠ Pe off target by " + ROUND(pe_error/1000, 1) + " km".
            PRINT "    Consider small RCS correction before Kerbin SOI".
        }
    } ELSE {
    ...
    IF SHIP:ORBIT:HASNEXTPATCH AND SHIP:ORBIT:NEXTPATCH:BODY = KERBIN {
        ...
        PRINT " ".
        PRINT "DEBUG INFO:".
        //PRINT "  Current pos:      " + ROUND(SHIP:GEOPOSITION:LAT, 2) + " / " + ROUND(SHIP:GEOPOSITION:LNG, 2).
