...
LOCAL PAD_TRIM_NORTH_M       IS 59.     // was 24 cross-pad trim offset
LOCAL PAD_TRIM_EAST_M        IS -38.     // -40 persistent west miss compensation (small extra east pre-bias)
...
LOCAL LAND_MAX_VEL_APPROACH   IS 12.0. // was 22.0 - half lateral speed cap.
...
FUNCTION targetNorthErrM {
    RETURN (PAD_LAT - SHIP:LATITUDE) * KERBIN_M_PER_DEG + PAD_TRIM_NORTH_M.
}

FUNCTION targetEastErrM {
    RETURN (PAD_LNG - SHIP:LONGITUDE) * KERBIN_M_PER_DEG * COS(SHIP:LATITUDE) + PAD_TRIM_EAST_M.
}
...
logEvent("1538 CFG RCSALT:" + RCS_ENABLE_ALT_M
...
PRINT "1538 CFG RCS:" + RCS_ENABLE_ALT_M + "/" + RCS_ENABLE_DIST_M
...
UNTIL phase = PH_TOUCHDOWN {
    ...
    ELSE IF phase = PH_AERO {
        ...
        IF SHIP:ALTITUDE > MID_RCS_MIN_ALT_M {
            SET aeroRcsStatus TO updateMidcourseRcsAssist(padGeo).
        } ELSE {
            clearRcsAssist().
