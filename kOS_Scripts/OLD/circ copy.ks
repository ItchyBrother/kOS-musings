...
// Version 5.0 - Revised node_to_orbit function.....THIS IS A SAVE PRIOR TO THAT.
...
FUNCTION node_to_orbit {
    ...
    IF engine_active AND SHIP:ORBIT:APOAPSIS < target_Ap - 500 {
    ...
    IF SHIP:ALTITUDE > 70000 AND SHIP:ORBIT:PERIAPSIS < 70000 {
        PRINT "Raising Periapsis to safe height." AT (0, 5).
        UNTIL ETA:APOAPSIS < 5 {
            PRINT "Time to Ap: " + ROUND(ETA:APOAPSIS, 1) + "s" AT (0, 6).
            WAIT 0.5.
        }
        ...
        UNTIL SHIP:ORBIT:PERIAPSIS >= target_Pe {
            ...
            SET current_throttle TO 0.01 * pe_error.
            SET current_throttle TO MAX(0.1, MIN(0.7, current_throttle)).
            LOCK THROTTLE TO current_throttle.

            PRINT "Pe: " + ROUND(SHIP:ORBIT:PERIAPSIS) AT (0, 6).
            WAIT 0.1.
        }

        LOCK THROTTLE TO 0.
        UNLOCK STEERING.
        PRINT "Periapsis is now " + ROUND(SHIP:ORBIT:PERIAPSIS) + " m." AT (0, 7).
        WAIT 10.
        RETURN.
    }
