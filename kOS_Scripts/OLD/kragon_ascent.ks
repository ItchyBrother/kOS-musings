...
FUNCTION PreDefinedKragon {
    ...
    GLOBAL kalcon_meco_ap  IS 80000.  // Ap (m) at which to cut Kalcon and hand off

    // Stage 2 (Kragon) targets — passed to circ.ks
    GLOBAL target_Ap       IS 80000.
    GLOBAL target_Pe       IS 80000.
...
FUNCTION SelectPad {
    ...
    UNTIL i >= PAD_NAMES:LENGTH - 1 {   // all entries except the last "Custom" slot
        PRINT (i + 1) + ". " + PAD_NAMES[i] + "   " + PAD_LATS[i] + " / " + PAD_LNGS[i] + " / " + PAD_ALTS[i] + " m".
        SET i TO i + 1.
    }
...
// ── Gravity turn pitch schedule (Kerbal scale) ──────────────
//  0 – turn_start_alt  : hold initial pitch
//  turn_start_alt – 15 km : ramp myPitchAngle → 30 deg
//  15 km – 45 km          : ramp 30 → 5 deg
//  > 45 km                : hold 0 deg (horizontal — let Ap build)
FUNCTION TargetPitch {
    LOCAL a IS SHIP:ALTITUDE.
    IF a <= turn_start_alt { RETURN myPitchAngle. }
    IF a < 15000 {
        LOCAL t IS Clamp((a - turn_start_alt) / (15000 - turn_start_alt), 0, 1).
        RETURN myPitchAngle + (30 - myPitchAngle) * t.
    }
    IF a < 45000 {
        LOCAL t IS Clamp((a - 15000) / (45000 - 15000), 0, 1).
        RETURN 30 + (0 - 30) * t.
    }
    RETURN 0.
}
...
FUNCTION WriteKalconParams {
    LOCAL kVol IS PROCESSOR("Kalcon"):VOLUME.
    // Remove stale params file if present
    IF kVol:EXISTS("rtls_params.ks") { kVol:DELETE("rtls_params.ks"). }
    LOCAL pf IS kVol:CREATE("rtls_params.ks").
    pf:WRITELN("// Auto-generated by kragon_ascent.ks at MECO").
    pf:WRITELN("SET kalcon_pad_lat TO " + pad_lat + ".").
    pf:WRITELN("SET kalcon_pad_lng TO " + pad_lng + ".").
    pf:WRITELN("SET kalcon_pad_alt TO " + pad_alt + ".").
    pf:FLUSH().
    PRINT "RTLS params written to Kalcon volume." AT (0, 20).
}
...
SAS ON.
RCS ON.
PRINT "Stage 2 MECO" AT (0, 9).
PRINT "Handing off to circ.ks..." AT (0, 10).
WAIT 3.
