// Diagnostic script to output modules, actions, and events for AM.MLP.TitanIILaunchStand and related parts
CLEARSCREEN.
PRINT "Diagnosing modules for AM.MLP.TitanIILaunchStand and related parts...".

// Open a file to save output
LOCAL outputFile IS "0:/titan_diagnostic.txt".
IF EXISTS(outputFile) { DELETEPATH(outputFile). }
LOG "Diagnosing modules for AM.MLP.TitanIILaunchStand and related parts..." TO outputFile.
...
// Test modules on AM.MLP.TitanIILaunchStand
IF SHIP:PARTSNAMED("AM.MLP.TitanIILaunchStand"):LENGTH > 0 {
    SET basePart TO SHIP:PARTSNAMED("AM.MLP.TitanIILaunchStand")[0].
    CLEARSCREEN.
    PRINT "Testing modules on AM.MLP.TitanIILaunchStand..." AT (0, 0).
    SET moduleIndex TO 0.
    SET continueTesting TO TRUE.
    FOR moduleName IN basePart:ALLMODULES {
        IF continueTesting {
            SET continueTesting TO testModule(basePart, "AM.MLP.TitanIILaunchStand", moduleName, moduleIndex, 1).
            SET moduleIndex TO moduleIndex + 1.
            CLEARSCREEN.
        }
    }
    IF moduleIndex = 0 {
        PRINT "No modules found on AM.MLP.TitanIILaunchStand" AT (0, 1).
        LOG "No modules found on AM.MLP.TitanIILaunchStand" TO outputFile.
    }
} ELSE {
...
// Test ModuleResourceDrain on LVDrain2
IF SHIP:PARTSTAGGED("LVDrain2"):LENGTH > 0 {
    SET drainPart TO SHIP:PARTSTAGGED("LVDrain2")[0].
    CLEARSCREEN.
    PRINT "Testing ModuleResourceDrain on LVDrain2..." AT (0, 0).
    SET moduleIndex TO 0.
    SET continueTesting TO TRUE.
    FOR moduleName IN drainPart:ALLMODULES {
        IF moduleName = "ModuleResourceDrain" AND continueTesting {
            SET continueTesting TO testModule(drainPart, "LVDrain2", moduleName, moduleIndex, 1).
            SET moduleIndex TO moduleIndex + 1.
            CLEARSCREEN.
        }
    }
    IF moduleIndex = 0 {
        PRINT "No ModuleResourceDrain found on LVDrain2" AT (0, 1).
        LOG "No ModuleResourceDrain found on LVDrain2" TO outputFile.
    }
} ELSE {
    PRINT "Error: No LVDrain2 part found." AT (0, 1).
    LOG "Error: No LVDrain2 part found." TO outputFile.
}

// Test ModuleResourceDrain on LVDrain1
IF SHIP:PARTSTAGGED("LVDrain1"):LENGTH > 0 {
    FOR drainPart IN SHIP:PARTSTAGGED("LVDrain1") {
        CLEARSCREEN.
        PRINT "Testing ModuleResourceDrain on LVDrain1 (" + drainPart:NAME + ")..." AT (0, 0).
        SET moduleIndex TO 0.
        SET continueTesting TO TRUE.
        FOR moduleName IN drainPart:ALLMODULES {
            IF moduleName = "ModuleResourceDrain" AND continueTesting {
                SET continueTesting TO testModule(drainPart, "LVDrain1 (" + drainPart:NAME + ")", moduleName, moduleIndex, 1).
                SET moduleIndex TO moduleIndex + 1.
                CLEARSCREEN.
            }
        }
        IF moduleIndex = 0 {
            PRINT "No ModuleResourceDrain found on LVDrain1 (" + drainPart:NAME + ")" AT (0, 1).
            LOG "No ModuleResourceDrain found on LVDrain1 (" + drainPart:NAME + ")" TO outputFile.
        }
    }
} ELSE {
    PRINT "Error: No LVDrain1 parts found." AT (0, 1).
    LOG "Error: No LVDrain1 parts found." TO outputFile.
}

// Test ModuleAnimateGenericExtra on AM.MLP.LaunchStandCrewElevatorGemini
IF SHIP:PARTSNAMED("AM.MLP.LaunchStandCrewElevatorGemini"):LENGTH > 0 {
    ...
    CLEARSCREEN.
    PRINT "Testing ModuleAnimateGenericExtra on AM.MLP.LaunchStandCrewElevatorGemini..." AT (0, 0).
    SET moduleIndex TO 0.
    SET continueTesting TO TRUE.
    FOR moduleName IN walkwayPart:ALLMODULES {
        IF moduleName = "ModuleAnimateGenericExtra" AND continueTesting {
            SET continueTesting TO testModule(walkwayPart, "AM.MLP.LaunchStandCrewElevatorGemini", moduleName, moduleIndex, 1).
            SET moduleIndex TO moduleIndex + 1.
            CLEARSCREEN.
        }
    }
...
PRINT "Diagnostic complete. Output saved to 0:/titan_diagnostic.txt." AT (0, 0).
