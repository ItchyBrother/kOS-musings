...
FUNCTION checkFirstStageMECO {
    ...
    // Use stage ready..  need to rewrite.
...
FUNCTION Mode1_Launch {
    PRINT "Mode 1 - Launch" AT (0, 1).
    
    WHEN SHIP:ALTITUDE > 300 THEN {
        PRINT "Roll Program" AT (0, 2).
        rollProgram().
    }
    
    WAIT UNTIL SHIP:ALTITUDE > 1500.
    PRINT "Starting Gravity Turn" AT (0, 3).
    adaptiveGravityTurn(80000).
  
    WAIT UNTIL MECO_HAPPENED.
    PRINT "MECO.  MAIN ENGINE CUT-OFF." AT (0, 4). 
    PRINT "MECO Detected at " + ROUND(SHIP:ALTITUDE, 0) + " meters." AT (0, 20).
    SET MODE TO 2.
}
...
FUNCTION Mode3_FinalOrbit {
    PRINT "Mode 3 - Final Orbital Insertions" AT (0, 7).
    AG2 ON.
    RCS ON.
...
FUNCTION safeStage {
    IF NOT STAGE:READY {
        PRINT "Stage not ready, waiting..." AT (0, 20).
...
FUNCTION adaptiveGravityTurn {
    ...
    SET peakDynamicPressure TO 0. // Moved from Mode1_Launch
    SET dynamicPressureMonitorActive TO TRUE. // Moved from Mode1_Launch
    ...
    UNTIL SHIP:APOAPSIS > targetAltitude {
        ...
        // Dynamic Pressure Monitoring (Moved from Mode1_Launch)
        IF currentAltitude < BODY:ATM:HEIGHT {
            ...
            IF dynamicPressureMonitorActive {
                PRINT "Dynamic Pressure: " + ROUND(currentDynamicPressure, 2) + " Pa" AT (0, 40).
            } ELSE {
                PRINT "Peak Dynamic Pressure: " + ROUND(peakDynamicPressure, 2) + " Pa" AT (0, 40).
            }
        }
...
FUNCTION jettisonTower {
    PRINT "Jettisoning Tower at " + ROUND(SHIP:ALTITUDE, 0) + " meters." AT (0, 21).
    AG1 ON.
}
...
FUNCTION circularizeOrbit {
    ...
    PRINT "Estimated Burn Duration: " + ROUND(estimated_burn_duration, 1) + " seconds" AT (0, 26).
    PRINT "DeltaV Needed for Primary Objective: " + ROUND(dv_needed, 0) + " m/s" AT (0, 27).
    PRINT "DeltaV for Safety Margin: " + ROUND(safety_deltav, 0) + " m/s" AT (0, 28).
    PRINT "Total DeltaV Required: " + ROUND(dv_needed + safety_deltav, 0) + " m/s" AT (0, 29).
    ...
    IF (SHIP:DELTAV:CURRENT - safety_deltav) > 0 {
        ...
        UNTIL burn_initiated {
            IF TIME:SECONDS >= burn_start_time {
                ...
                LOCK THROTTLE TO 1.0.
                PRINT "Burn Start Time Before Ap: " + ROUND(ETA:APOAPSIS, 1) + " seconds" AT (0, 30).
            } ELSE IF TIME:SECONDS >= (burn_start_time - 10) {  // Start a small burn 10 seconds earlier to fine-tune
                LOCK THROTTLE TO 0.1.
                PRINT "Pre-Burn Adjustment Starting" AT (0, 31).
            }
            WAIT 0.01.
        }
        ...
        UNTIL (SHIP:PERIAPSIS >= min_periapsis AND ABS(SHIP:APOAPSIS - SHIP:PERIAPSIS) < 2000) OR (start_deltav - SHIP:DELTAV:CURRENT) > (start_deltav - (safety_deltav + 50)) {
            ...
            PRINT "Pitch Adjustment: " + pitch_adjustment + " | ETA Apoapsis: " + ROUND(ETA:APOAPSIS, 2) + " | Current Ap: " + ROUND(SHIP:APOAPSIS, 0) + " m | Apoapsis Passed: " + apoapsis_passed AT (0, 32).
        ...
        PRINT "Circularization Burn Completed." AT (0, 20).
        PRINT "Achieved Periapsis: " + ROUND(SHIP:PERIAPSIS, 0) + " meters." AT (0, 21).
        PRINT "Achieved Apoapsis: " + ROUND(SHIP:APOAPSIS, 0) + " meters." AT (0, 22).
        PRINT "Remaining DeltaV: " + ROUND(SHIP:DELTAV:CURRENT, 0) + " m/s." AT (0, 23).
        PRINT "Safety Margin: " + ROUND(safety_deltav, 0) + " m/s." AT (0, 24).
        PRINT "Actual Burn Duration: " + ROUND(actual_burn_duration, 1) + " seconds" AT (0, 31).
        
        IF SHIP:DELTAV:CURRENT < safety_deltav {
            PRINT "WARNING: Insufficient DeltaV for safe deorbit!" AT (0, 25).
        }
    } ELSE {
        PRINT "Not enough DeltaV to safely circularize orbit." AT (0, 20).
    }
}

// === User Interfacing ===
FUNCTION printFinalOrbitData {
    PRINT "Final Orbital Parameters:" AT (0, 25).
    PRINT "Apoapsis: " + ROUND(SHIP:APOAPSIS, 0) + " m" AT (0, 26).
    PRINT "Periapsis: " + ROUND(SHIP:PERIAPSIS, 0) + " m" AT (0, 27).
    PRINT "Inclination: " + ROUND(SHIP:ORBIT:INCLINATION, 2) + " degrees" AT (0, 28).
    PRINT "Eccentricity: " + ROUND(SHIP:ORBIT:ECCENTRICITY, 3) AT (0, 29).
    PRINT "Remaining Liquid Fuel: " + ROUND(SHIP:LIQUIDFUEL, 0) + " units" AT (0, 30).
    PRINT "Remaining Delta-V: " + ROUND(SHIP:DELTAV:CURRENT, 0) + " m/s" AT (0, 31).
}
