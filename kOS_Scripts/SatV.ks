...
// Countdown Clock
PRINT "========================================" AT (0, 1).
PRINT "SATURN V LAUNCH SEQUENCE" AT (0, 0).
FROM {local countdown is myCountdownTime.} UNTIL countdown <= 0 STEP {SET countdown to countdown - 1.} DO {
    ...
    IF countdown = 9 AND NOT ignitionStarted {
        PRINT "Ignition Sequence Start          " AT (0, 15).
        ...
        Staging(0.5).
        PRINT "S-IC Engines Igniting...         " AT (0, 16).
        ...
        WHEN THROTTLE >= 0.70 AND myStage > 0 THEN {
            PRINT "Tower Arms Retracting...         " AT (0, 17).
            UNTIL myStage <= 0 {
                Staging(0.1).
                SET myStage TO myStage - 1.
            }
        ...
        IF SHIP:AVAILABLETHRUST <= 0 {
            PRINT "IGNITION FAILURE - CLAMPS HOLDING" AT (0, 18).
            LOCK THROTTLE TO 0.
        }
    }
...
LOCAL maxQReached IS FALSE. // FLAG FOR MAX-Q Call
LOCAL Clrlin2 IS FALSE. // FLAG FOR clearing line 2 after MAX-Q Call.
PRINT "Mode 2 - Ascent/Gravity Turn    " AT (0, 0).
PRINT "                " AT (0, 2).  // CLEARS LIFT-OFF From screen.

// Main ascent loop - handles both gravity turn AND staging checks
UNTIL SECONDSTAGE AND SHIP:ALTITUDE > 40000 {
    
    // Convert dynamic pressure from atm to kPa
    LOCAL dynPressKpa IS SHIP:DYNAMICPRESSURE * 101.325.
    
    // Call out Max Q when it exceeds 19 kPa
    IF NOT maxQReached AND dynPressKpa >= 18.5 {
        PRINT "MAX Q                            " AT (0, 2).
    ...
    IF SHIP:ALTITUDE >= 2000 AND SHIP:ALTITUDE < 25000 {
        ...
        PRINT "Pitch:     " + ROUND(pitchAngle, 1) + "°     " AT (0, 9).
    }

    IF NOT Clrlin2 AND SHIP:ALTITUDE >= 11000 {
        PRINT "                 " AT (0, 2).
        SET Clrlin2 TO TRUE.
    }
    ...
    IF SHIP:ALTITUDE >= 35000 {
        ...
        PRINT "Pitch:     40.0° (Heads Up Roll)" AT (0, 9).
    }
    ...
    IF FIRSTSTAGE AND NOT centerEngineCut AND STAGE:LIQUIDFUEL < (STAGE:RESOURCESLEX["LiquidFuel"]:CAPACITY * 0.07) {
        PRINT "S-IC Center Engine Cutoff        " AT (0, 15).
    ...
    IF FIRSTSTAGE AND NOT mecoPrep AND STAGE:LIQUIDFUEL < (STAGE:RESOURCESLEX["LiquidFuel"]:CAPACITY * 0.03) {
        PRINT "Preparing for MECO               " AT (0, 16).
    ...
    IF FIRSTSTAGE AND STAGE:LIQUIDFUEL < 0.1 AND STAGE:OXIDIZER < 0.1 {
        PRINT "MECO - S-IC Shutdown             " AT (0, 17).
        LOCK THROTTLE TO 0.
        Staging(0.5).  // FIRST AND SECOND STAGE SEPERATION.
        PRINT "S-IC Stage Separation            " AT (0, 18).
        WAIT 2.
        LOCK THROTTLE TO 1.  // SECOND STAGE THROTTLE UP.
         PRINT "S-II Stage Ignition              " AT (0, 19).
        Staging(2).  // Tower Jettison
        PRINT "Launch Escape Tower Jettison     " AT (0, 20).
        Staging(3). 
        //Staging(1).
        PRINT "S-II Interstage Jettison         " AT (0, 21).    
...
// Reenforcing the steering, pitch and roll after the until.
// It shouldn't be necessary, but just in case.
LOCK STEERING TO HEADING(myAzimuth, 40, (myRoll -180)).

// Second stage burnout
//WAIT UNTIL SECONDSTAGE AND STAGE:LIQUIDFUEL < 0.1 OR SHIP:APOAPSIS >= 115000. 
// UNTIL SECONDSTAGE AND STAGE:LIQUIDFUEL < 0.1 OR SHIP:APOAPSIS >= 115000 {
//     //TWR Calculations
...
//     PRINT "Periapsis: " + ROUND(SHIP:PERIAPSIS/1000, 1) + " km     " AT (0, 6).
//     PRINT "Velocity:  " + ROUND(SHIP:VELOCITY:SURFACE:MAG) + " m/s     " AT (0, 7).
//     PRINT "TWR:       " + ROUND(TWR, 2) + "G        " AT (0, 8).
//     WAIT 0.1.
// }
// PRINT "SECO - S-II Shutdown             " AT (0, 16).
// LOCK THROTTLE TO 0.
// Staging(1).
// LOCK THROTTLE TO 1.
// PRINT "S-II Stage Separation            " AT (0, 17).
// Staging(3).
// RCS ON.
// PRINT "S-IVB Stage Ignition             " AT (0, 18).
// WAIT 3.
// RCS OFF.  // Turns on RCS for 3rd STAGE.
// AG8 ON. // Toggles Ullage motors on third stage.  TURNS THEM OFF.
// SET SECONDSTAGE TO FALSE.
// PRINT "Ascent Phase Complete            " AT (0, 22).

// PHASE 1: Burn to apoapsis target
PRINT "Phase 1: Burning to Apoapsis     " AT (0, 15).
UNTIL SHIP:APOAPSIS >= 115000 {
    //TWR Calculations
    SET TWR TO SHIP:AVAILABLETHRUST / (SHIP:MASS * 9.81).
    //Update flight data display.
    PRINT "Altitude:  " + ROUND(SHIP:ALTITUDE/1000, 2) + " km     " AT (0, 4).
...
// PHASE 2: Raise periapsis using pitch control
PRINT "Phase 2: Raising Periapsis       " AT (0, 15).
SET TARGET_PE2 TO 30000.  // 30 km target
SET TARGET_AP2 TO 115000. // Hold at 115 km

UNTIL SHIP:PERIAPSIS >= TARGET_PE2 OR STAGE:LIQUIDFUEL < 0.1 OR SHIP:APOAPSIS >= target_Ap - 5000 {
    // Dynamic pitch control to maintain Ap around 115 km
    SET AP_ERROR TO SHIP:APOAPSIS - TARGET_AP2.
    
    IF AP_ERROR > 5000 {
        SET TARGET_PITCH TO -15.  // Way too high, go nearly horizontal
    } ELSE IF AP_ERROR > 2000 {
        SET TARGET_PITCH TO -10.  // Too high, shallow angle
    } ELSE IF AP_ERROR > 500 {
        SET TARGET_PITCH TO 0.  // Slightly high
    } ELSE IF AP_ERROR < -2000 {
        SET TARGET_PITCH TO 30.  // Getting low, pitch up more
    } ELSE {
        SET TARGET_PITCH TO 20.  // Sweet spot
    }
    
    LOCK STEERING TO HEADING(myAzimuth, TARGET_PITCH).  // East at calculated pitch
    ...
    PRINT "Velocity:  " + ROUND(SHIP:VELOCITY:SURFACE:MAG) + " m/s     " AT (0, 7).
    PRINT "TWR:       " + ROUND(TWR, 2) + "G        " AT (0, 8).
    PRINT "Pitch:     " + ROUND(TARGET_PITCH, 1) + "°        " AT (0, 9).
    WAIT 0.1.
}
...
SET MODE TO 3.
PRINT "Mode 3 - Burn to Target Ap       " AT (0, 0).

// Continue burning until apoapsis reaches target
UNTIL SHIP:ORBIT:APOAPSIS >= target_Ap {
    PRINT "Target Ap: " + ROUND(target_Ap/1000, 1) + " km     " AT (0, 5).
    PRINT "Current:   " + ROUND(SHIP:ORBIT:APOAPSIS/1000, 1) + " km     " AT (0, 6).
    WAIT 0.1.
}

// Cut throttle when we hit target apoapsis
LOCK THROTTLE TO 0.
AG9 OFF. //SHUT OFF S-IVB Engine.
PRINT "S-IVB Cutoff - Target Ap Reached " AT (0, 19).
PRINT "Ap: " + ROUND(SHIP:ORBIT:APOAPSIS/1000, 1) + " km     " AT (0, 20).
PRINT "Pe: " + ROUND(SHIP:ORBIT:PERIAPSIS/1000, 1) + " km     " AT (0, 21).
...
WHEN HASNODE AND NEXTNODE:ETA <= 60 THEN {  // Catch it early
    ...
    IF ABS(currentNodeTime - lastNodeTime) > 30 {
        ...
        LOCK THROTTLE TO 0.
        AG9 ON.
        LOCAL burnDuration IS NEXTNODE:DELTAV:MAG / (SHIP:MAXTHRUST / SHIP:MASS).
        ...
        PRINT "Burn Duration: " + ROUND(burnDuration, 1) + "s     " AT (0, 11).
        PRINT "Burn Start ETA: T-" + ROUND(burnStartETA, 1) + "s     " AT (0, 12).
        PRINT "Ullage Start ETA: T-" + ROUND(ullageStartETA, 1) + "s     " AT (0, 13).
        
        // Wait until it's time for ullage
        WAIT UNTIL NEXTNODE:ETA <= ullageStartETA.
        
        PRINT "S-IVB Shutdown for Ullage        " AT (0, 15).
        RCS ON.
        AG9 OFF.  // Turn off S-IVB J2 Engine
        WAIT 1.
        LOCK THROTTLE TO 1.  //making sure they fire!
        PRINT "Ullage Motors Firing             " AT (0, 16).
        AG8 ON.  // Fire ullage motors
        WAIT 5.
        
        PRINT "S-IVB Restart                    " AT (0, 17).
        AG9 ON.  // Restart main engine
        WAIT 3.
        AG8 OFF.  // Shut off ullage
        RCS OFF.
...
CLEARSCREEN.
PRINT "========================================" AT (0, 1).
PRINT "ORBIT CLEANUP                    " AT (0, 0).
PRINT "Apoapsis:  " + ROUND(SHIP:ORBIT:APOAPSIS/1000, 1) + " km     " AT (0, 4).
PRINT "Periapsis: " + ROUND(SHIP:ORBIT:PERIAPSIS/1000, 1) + " km     " AT (0, 5).
PRINT "S-IVB Shutdown                   " AT (0, 15).
WAIT 10.
AG9 OFF.  // Shuts down SIV-B J2 Engine.

WAIT 30.
SET MODE TO 5.
CLEARSCREEN.
PRINT "========================================" AT (0, 3).
PRINT "Mode 5 - Orbit Operations        " AT (0, 4).
PRINT "Final Orbital Parameters:        " AT (0, 5).
...
FUNCTION SetupAbortDetection {
    ...
    WHEN TRUE THEN {
        IF SHIP:ALTITUDE < 21000 {
            ...
            PRINT "Abort Detection Disabled (>20km)         " AT (0, 25).
            RETURN FALSE.
        }
    }
}
