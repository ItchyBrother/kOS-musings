...
FUNCTION UPDATE_DISPLAY {
    ...
    IF current_ralt >= 0 {
        IF current_ralt > 150 {
            PRINT " NEXT: HIGH GATE at 150m           " AT (0,26).
        } ELSE IF current_ralt > 15 {
            PRINT " NEXT: LOW GATE at 15m             " AT (0,26).
...
// ============================================================================
// P64 - APPROACH PHASE (VISIBILITY PHASE)
// ============================================================================
...
FUNCTION PROGRAM_64 {
    ...
    // Redesignation offset (meters from original target)
    LOCAL target_offset_north IS 0.
    LOCAL target_offset_east IS 0.
    LOCAL redesignation_active IS FALSE.
    
    LOCAL approach_complete IS FALSE.
    LOCAL steering_locked IS FALSE.
    ...
    UNTIL approach_complete {
        ...
        // STEERING: Lock once horizontal velocity is killed to prevent flips
        IF NOT steering_locked {
            IF vel_horizontal > 5 {
                // Still moving fast - point retrograde
                LOCK STEERING TO SHIP:SRFRETROGRADE.
            } ELSE IF vel_horizontal > 1 {
                // Slowing down - blend to vertical
                LOCAL up_vec IS SHIP:UP:VECTOR.
                LOCAL retro_vec IS SHIP:SRFRETROGRADE:VECTOR.
                LOCAL blend_factor IS vel_horizontal / 5.
                LOCAL blend_vec IS (retro_vec * blend_factor) + (up_vec * (1 - blend_factor)).
                LOCK STEERING TO blend_vec:NORMALIZED.
            } ELSE {
                // FIRST: Point straight UP (not blended)
                LOCK STEERING TO LOOKDIRUP(SHIP:UP:VECTOR, P64_INITIAL_FACING).
                
                // THEN: Lock steering on next loop iteration
                // Give it one loop to actually point UP before locking
                IF VANG(SHIP:FACING:VECTOR, SHIP:UP:VECTOR) < 10 {
                    // Now we're actually pointing up - safe to lock
                    SET steering_locked TO TRUE.
                    
                    // Apply redesignation if exists
                    IF target_offset_north <> 0 OR target_offset_east <> 0 {
                        LOCAL north_vec IS SHIP:NORTH:VECTOR.
                        LOCAL east_vec IS VCRS(SHIP:UP:VECTOR, north_vec).
                        LOCAL offset_vec IS (north_vec * target_offset_north) + (east_vec * target_offset_east).
                        
                        LOCAL to_target IS TARGET_POSITION:POSITION - SHIP:GEOPOSITION:POSITION + offset_vec.
                        
                        IF to_target:MAG > 5 {
                            LOCAL correction_strength IS MIN(0.3, to_target:MAG / 100).
                            LOCAL correction_vec IS to_target:NORMALIZED * correction_strength * 10.
                            LOCAL steering_vec IS SHIP:UP:VECTOR + correction_vec.
                            LOCK STEERING TO LOOKDIRUP(steering_vec:NORMALIZED, P64_INITIAL_FACING).
                        }
                    }
                }
            }
        ...
        // TARGET: Zero velocity at Low Gate
        LOCAL alt_above_low_gate IS ralt - MEM_CONFIG["LOW_GATE_ALT"].

        LOCAL target_vert_vel IS 0.  // Default: stopped at Low Gate

        IF alt_above_low_gate > 1500 {
            // High up: 40-50 m/s 
            SET target_vert_vel TO -40.0 - (alt_above_low_gate - 1500) / 150.
        } ELSE IF alt_above_low_gate > 800 {
            // Mid: 30-40 m/s
            SET target_vert_vel TO -30.0 - (alt_above_low_gate - 800) / 87.5.
        } ELSE IF alt_above_low_gate > 300 {
            // Getting close: 20-30 m/s
            SET target_vert_vel TO -20.0 - (alt_above_low_gate - 300) / 71.4.
        } ELSE IF alt_above_low_gate > 50 {
            // Final approach: 10-20 m/s
            SET target_vert_vel TO -10.0 - (alt_above_low_gate - 50) / 62.5.
        } ELSE {
            // At Low Gate: hovering (0 m/s)
            SET target_vert_vel TO -5.0.
        }

        // STRONG correction
        LOCAL vert_error IS vel_vertical - target_vert_vel.
        LOCAL throttle_cmd IS hover_throttle - (vert_error * 0.4).

        // Allow aggressive throttle
        LOCAL min_throttle IS MAX(0.05, hover_throttle * 0.2).
        LOCAL max_throttle IS MIN(0.98, hover_throttle * 2.0).
        SET throttle_cmd TO MAX(min_throttle, MIN(max_throttle, throttle_cmd)).

        SAFE_THROTTLE(throttle_cmd).

        //PRINT "Alt above LG: " + ROUND(alt_above_low_gate, 0) + "m  Tgt V: " + ROUND(target_vert_vel, 1) + " m/s        " AT (0,27).
        
        ALTITUDE_CALLOUTS().
        
        // REDESIGNATION MODE - Only between High Gate and Low Gate
        IF HIGH_GATE_PASSED AND NOT LOW_GATE_PASSED {
            IF TERMINAL:INPUT:HASCHAR {
                LOCAL cmd IS TERMINAL:INPUT:GETCHAR().
                
                IF cmd = "h" OR cmd = "H" {
                    SET redesignation_active TO NOT redesignation_active.
                    IF redesignation_active {
                        CALLOUT("REDESIGNATION MODE").
                    } ELSE {
                        CALLOUT("REDESIGNATION OFF").
                    }
                }
                
                IF redesignation_active {
                    IF cmd = "i" OR cmd = "I" {
                        SET target_offset_north TO target_offset_north + 10.
                        CALLOUT("TARGET FORWARD 10M").
                    } ELSE IF cmd = "k" OR cmd = "K" {
                        SET target_offset_north TO target_offset_north - 10.
                        CALLOUT("TARGET BACK 10M").
                    } ELSE IF cmd = "j" OR cmd = "J" {
                        SET target_offset_east TO target_offset_east - 10.
                        CALLOUT("TARGET LEFT 10M").
                    } ELSE IF cmd = "l" OR cmd = "L" {
                        SET target_offset_east TO target_offset_east + 10.
                        CALLOUT("TARGET RIGHT 10M").
                    }
                }
            }
            
            // Display redesignation status
            IF redesignation_active {
                PRINT "[REDESIGNATION ACTIVE - IJKL TO ADJUST]     " AT (0,25).
                PRINT "Offset: N" + target_offset_north + "m E" + target_offset_east + "m         " AT (0,26).
            } ELSE IF target_offset_north <> 0 OR target_offset_east <> 0 {
                PRINT "[H] for Redesignation - Offset: N" + target_offset_north + "m E" + target_offset_east + "m    " AT (0,25).
            } ELSE {
                PRINT "[H] for Redesignation Mode                  " AT (0,25).
            }
        }
        
        // Exit when low AND horizontal nearly zero
        IF ralt < MEM_CONFIG["LOW_GATE_ALT"] { //AND vel_horizontal < 5 {
...
// ============================================================================
// P66 - TERMINAL DESCENT (AUTOMATIC LANDING)
// ============================================================================
...
FUNCTION PROGRAM_66 {
    ...
    // IMMEDIATE DESCENT - No velocity killing needed, P64 already did it
    CALLOUT("FINAL DESCENT - 6 m/s").
    
    LOCAL target_descent_rate IS -6.0. //  Was -3 now -5.
    IF ALT:RADAR < 50 {
        SET target_descent_rate TO -3.0.
    }
    ...
    UNTIL ALT:RADAR <= MEM_CONFIG["ENGINE_CUTOFF_ALT"] AND ALT:RADAR > 0 {
        ...
        IF ralt < 30 {  // Use ralt variable
            SET target_descent_rate TO -2.0.
        }
        IF ralt < 10 {  // Use ralt variable
        ...
        // Correction for target descent rate
        ...
        LOCAL fuel_pct IS GET_FUEL_PERCENT().
        IF fuel_pct < 5 {
            PRINT "█████ FUEL EXHAUSTION IMMINENT █████" AT (0,0).
        }
