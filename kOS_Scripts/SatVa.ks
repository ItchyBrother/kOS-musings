...
UNTIL SECONDSTAGE AND SHIP:ALTITUDE > 40000 {
    ...
    IF SHIP:ALTITUDE >= 35000 {
        LOCK STEERING TO HEADING(myAzimuth, 40, myRoll).
    ...
    IF FIRSTSTAGE AND STAGE:LIQUIDFUEL < 0.1 AND STAGE:OXIDIZER < 0.1 {
       ...
       // PRINT "Launch Escape Tower Jettison     " AT (0, 20).
        ...
        //SAS OFF.
        //WAIT 0.2.
        LOCK STEERING TO HEADING(myAzimuth,40).
        WAIT 1.
    }
...
WHEN SHIP:ALTITUDE >=72000 THEN {
    Staging(1).
}

// Mode 4 - Orbit Circulation
SET MODE TO 4.
...
IF SHIP:PERIAPSIS < target_Pe - 1000 {
...
PRINT ">>>>> ORBIT OPERATIONS <<<<<" AT (0, 15).
WAIT 5.
Debug().
PROCESSOR("S-IIC"):ACTIVATE().
PRINT "Staring Orbit operations"  AT (0, 18).
WAIT 2.
PRINT "SKylab seperating from SIC-Stage." AT (0, 20).
Staging(1).
WAIT 10.
PRINT "Activating Skylab" AT (0,21).
AG1 ON.
RADIATORS ON.
WAIT 90.
LIGHTS ON.
PRINT "Skylabe activated -- ENDING PROGRAM" AT (0, 25).

PRINT ">>>>>> PROGRAM END <<<<<<<" AT (0, 30).
...
FUNCTION Debug{
    PRINT "Press any key to continue...." AT (0, 16).
    SET dummy TO TERMINAL:INPUT:GETCHAR().
    WAIT 1.
    UNSET dummy.
}
...
FUNCTION PreDefined {
    ...
    GLOBAL myAzimuth TO 50.            // Azimuth (90 = due east)
    GLOBAL myRoll TO 0.                // Roll
    GLOBAL target_Ap TO 160000.        // Target Apoapsis
    GLOBAL target_Pe TO 155000.        // Target Periapsis
...
FUNCTION prepareLaunch {
    ...
    IF choice = "P" OR choice = "p" {
        ...
        PRINT "Press 'P' to proceed or 'A' to abort" AT (0, 25).
