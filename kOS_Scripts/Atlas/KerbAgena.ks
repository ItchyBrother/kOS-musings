// KOS Script for Rocket Launch Orbital Flight (Atlas Agena)
...
SET payload_jet TO FALSE.
IF SHIP:ALTITUDE > 70000 {
    PRINT "Payload faring jettison!" AT (0, 16).
    Staging(2).
    SET payload_jet TO TRUE.
}
...
PRINT "Shutting down sustainer engine." AT (0,14).
AG7 ON. // Assuming AG7 turns sustainer off
WAIT 5.
// Activate payload if not already.
IF NOT payload_jet{
    PRINT "Payload faring jettison!" AT (0,16).
    Staging(2).
}

// Add for MUN launches - Need to comment out if any thing else.
PRINT "PLEASE PRESS CTRL+C to Kill processor" AT (0, 17).
WAIT 60.
// Add for MUN launches - Need to comment out if any thing else.

PRINT "Activating payload computer." AT (0,17).
PROCESSOR("CommSat"):ACTIVATE().  //Activating the satellite processor..
// Sending parameters to Satellite processor
// Need antenna for this work on the booster?
SET Satconfig TO LIST ("SEPARATION", target_Ap, target_Pe, bypassmenu).
LOCAL mysat IS PROCESSOR("CommSat").
IF mysat:CONNECTION:SENDMESSAGE(Satconfig) {
    PRINT "Message Sent.".
} ELSE {
    PRINT "ERROR!".
}

AG7 OFF.  // Turning Sustainer Engine back on.
run kAtlasStage.
WAIT 5.
// Final clean up.
SHUTDOWN.
