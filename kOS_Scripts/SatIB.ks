...
// Version 1.0 - Taking GTitan script and modifying for Saturn IB.
...
FROM {local countdown is myCountdownTime.} UNTIL countdown <= 0 STEP {SET countdown to countdown - 1.} DO {
   ...
   // Check for ignition sequence start
    IF countdown = 8 AND NOT ignitionStarted {
        ...
        // Mark ignition as started - this will trigger the smooth ramp
        SET ignitionStarted TO TRUE.
        SET ignitionStartTime TO TIME:SECONDS.
        
        IF SHIP:AVAILABLETHRUST <= 0 {
            PRINT "Ignition failure – clamps holding." AT (0,6).
            LOCK THROTTLE TO 0.
        }
    }
...
//LOCK STEERING TO LOOKDIRUP( HEADING(90, 87):VECTOR, SHIP:FACING:TOPVECTOR ).  
...
//PRINT "Liquid Fuel is: " + STAGE:LIQUIDFUEL AT (0,30).
//SHIP:LIQUIDFUEL FULL IS 14310,  SHIP:OXIDIZER IS 17490
//After first stage: LIQUID 3601 - ,  OXIDIZER - 4401
// Aligning prior to MECO to velocity vector at 5% fuel left
WHEN FIRSTSTAGE AND STAGE:LIQUIDFUEL < (STAGE:RESOURCESLEX["LiquidFuel"]:CAPACITY * 0.07) THEN {
    PRINT "Preparing for Main Engine Cut-off." AT (0, 11).
    LOCK STEERING TO PROGRADE.
    WAIT 0.1.  // Physic tick.  
}

// Set up staging trigger (runs independently of altitude)
WHEN FIRSTSTAGE AND SHIP:LIQUIDFUEL < 3601 AND SHIP:OXIDIZER < 4401 THEN {
    // Perform first stage shutdown and staging
    PRINT "MECO!" AT (0, 12).
    Staging(0.1).
    ...
    WAIT 3.
    AG8 ON.
    PRINT "Second stage ignition!" AT (0, 14).
    Staging(1).
    PRINT "Escape tower jettison." AT (0, 15).
    SET FIRSTSTAGE TO FALSE.
}

// Phase 1: Pitch over between 2 km and 30 km
WHEN SHIP:ALTITUDE >= 2000 THEN {
    UNTIL SHIP:ALTITUDE >= 30000 {
        SET pitchAngle TO myPitchAngle - ((SHIP:ALTITUDE - 2000) * (40 / 28000)). // 88° to 40°
...
// Phase 2: Pitch control 45° → 5° between 30–40 km
...
LOCK THROTTLE TO 0.
PRINT "Target apoapsis reached - SECO!" AT (0, 22).
...
FUNCTION SetupAbortDetection {
    ...
    WHEN TRUE THEN {
        IF SHIP:ALTITUDE < 30000 {
            ...
            PRINT "Abort detection disabled (above 30km)" AT (0, 23).
            RETURN FALSE.
        }
    }
}
...
FUNCTION prepareLaunch {
    ...
    // FOR SATIB STAND ONLY - COMMENT OUT IF NOT USED.
    // SET basePart TO 0.
    // FOR p IN SHIP:PARTS {
    //     IF p:NAME = "AM_MLP_LargeLaunchStand" OR p:TITLE:TOLOWER():CONTAINS("Saturn IB Launch Stand") {
    //         SET basePart TO p.
    //         BREAK.
    //     }
    // }
    // IF basePart = 0 {
    //     PRINT "Error: No Saturn IB Launch Stand found." AT (0, 1).
    //     WAIT 5.
    //     REBOOT.
    // }
    // FOR SATIB STAND ONLY - COMMENT OUT IF NOT USED.
    ...
    FOR p IN sivbParts {
        FOR res IN p:RESOURCES {
            IF res:NAME = "LiquidFuel" { SET sivb_lfCap TO sivb_lfCap + res:CAPACITY. }
            IF res:NAME = "Oxidizer" { SET sivb_oxCap TO sivb_oxCap + res:CAPACITY. }
        }
    }
    ...
    // Error check
    IF sivbParts:LENGTH = 0 {
        PRINT "WARNING: No parts tagged 'SIVB' found!" AT (0, currentLine).
        SET currentLine TO currentLine + 1.
        PRINT "Will fuel everything to 100% but cannot drain SIVB to target." AT (0, currentLine).
        SET currentLine TO currentLine + 1.
        PRINT "Tag your S-IVB tank with 'SIVB' for partial fueling." AT (0, currentLine).
        SET currentLine TO currentLine + 1.
        WAIT 5.
    }
    
    IF sivbParts:LENGTH > 0 AND (sivb_lfCap <= 0 OR sivb_oxCap <= 0) {
        PRINT "ERROR: SIVB tagged parts don't contain LF/OX!" AT (0, currentLine).
        WAIT 5.
        REBOOT.
    }
    ...
    IF sivbParts:LENGTH > 0 {
        PRINT "Step 2: Drain SIVB to " + SIVB_TARGET_PERCENT + "%" AT (0, currentLine).
        SET currentLine TO currentLine + 1.
        PRINT "" AT (0, currentLine).
        SET currentLine TO currentLine + 1.
        PRINT "SIVB Tank - LF: " + ROUND(sivb_lfCap) + "  OX: " + ROUND(sivb_oxCap) + " (" + sivbParts:LENGTH + " parts)" AT (0, currentLine).
        SET currentLine TO currentLine + 1.
        PRINT "SIVB Target: " + SIVB_TARGET_PERCENT + "% (" + ROUND(sivb_lfCap * SIVB_TARGET_PERCENT/100) + " LF, " + ROUND(sivb_oxCap * SIVB_TARGET_PERCENT/100) + " OX)" AT (0, currentLine).
        SET currentLine TO currentLine + 1.
        PRINT "SIVB Drain valves found: " + sivbValveDrains:LENGTH AT (0, currentLine).
        SET currentLine TO currentLine + 1.
    }
    PRINT "" AT (0, currentLine).
    SET currentLine TO currentLine + 1.
    ...
    // --- SIVB Drain Section ---
    IF sivbParts:LENGTH > 0 AND sivbValveDrains:LENGTH > 0 {
        CLEARSCREEN.
        PRINT "=== SIVB DRAIN TO " + SIVB_TARGET_PERCENT + "% ===" AT (0, 0).
        PRINT "" AT (0, 1).
        
        // Helper function to get SIVB fuel
        FUNCTION GetSIVBFuel {
            PARAMETER resName.
            LOCAL total IS 0.
            FOR p IN sivbParts {
                FOR res IN p:RESOURCES {
        ...
        SET sivb_lfTarget TO sivb_lfCap * (SIVB_TARGET_PERCENT / 100).
        SET sivb_oxTarget TO sivb_oxCap * (SIVB_TARGET_PERCENT / 100).
        
        PRINT "Current SIVB: LF " + ROUND(GetSIVBFuel("LiquidFuel")) + "  OX " + ROUND(GetSIVBFuel("Oxidizer")) AT (0, 2).
        PRINT "Target SIVB: LF " + ROUND(sivb_lfTarget) + "  OX " + ROUND(sivb_oxTarget) AT (0, 3).
        PRINT "" AT (0, 4).
        PRINT "Starting SIVB drain..." AT (0, 5).
        PRINT "Press '9' to stop drain early" AT (0, 6).
        
        // Start only SIVB drains
        FOR i IN RANGE(0, sivbValveDrains:LENGTH) {
            LOCAL d IS sivbValveDrains[i].
            LOCAL cm IS SIVB_DRAIN_CMDS[i].
            DO_CMD(d, cm["start"]).
        }
        PRINT "SIVB drain valves started." AT (0, 7).
        PRINT "" AT (0, 8).
        
        // Monitor SIVB levels
        LOCAL drainLine IS 9.
        LOCAL drainAborted IS FALSE.
        UNTIL FALSE {
            LOCAL sivb_lf IS GetSIVBFuel("LiquidFuel").
            LOCAL sivb_ox IS GetSIVBFuel("Oxidizer").
            
            // Check if reached target
            IF sivb_lf <= sivb_lfTarget AND sivb_ox <= sivb_oxTarget {
                PRINT "SIVB drain complete!                    " AT (0, drainLine).
                SET drainLine TO drainLine + 1.
                BREAK.
            }
            
            // Display progress
            LOCAL sivb_lfPct IS 0.
            LOCAL sivb_oxPct IS 0.
            IF sivb_lfCap > 0 { SET sivb_lfPct TO ROUND((sivb_lf / sivb_lfCap) * 100). }
            IF sivb_oxCap > 0 { SET sivb_oxPct TO ROUND((sivb_ox / sivb_oxCap) * 100). }
            
            PRINT "SIVB Draining: LF " + ROUND(sivb_lf) + " (" + sivb_lfPct + "%)  OX " + ROUND(sivb_ox) + " (" + sivb_oxPct + "%)    " AT (0, drainLine).
            
            IF TERMINAL:INPUT:HASCHAR {
                SET k TO TERMINAL:INPUT:GETCHAR().
                IF k = "9" {
                    PRINT "Drain stopped by user                   " AT (0, drainLine + 1).
                    SET drainLine TO drainLine + 2.
                    SET drainAborted TO TRUE.
                    BREAK.
                }
            }
            WAIT 0.5.
        ...
        // Final report
        LOCAL final_sivb_lf IS GetSIVBFuel("LiquidFuel").
        LOCAL final_sivb_ox IS GetSIVBFuel("Oxidizer").
        LOCAL final_sivb_lf_pct IS 0.
        LOCAL final_sivb_ox_pct IS 0.
        IF sivb_lfCap > 0 { SET final_sivb_lf_pct TO ROUND((final_sivb_lf/sivb_lfCap)*100). }
        IF sivb_oxCap > 0 { SET final_sivb_ox_pct TO ROUND((final_sivb_ox/sivb_oxCap)*100). }
        
        PRINT "" AT (0, drainLine).
        SET drainLine TO drainLine + 1.
        PRINT "Final SIVB: LF " + ROUND(final_sivb_lf,1) + " (" + final_sivb_lf_pct + "%)  OX " + ROUND(final_sivb_ox,1) + " (" + final_sivb_ox_pct + "%)" AT (0, drainLine).
        SET drainLine TO drainLine + 1.
        PRINT "Total Vehicle: LF " + ROUND(SHIP:LIQUIDFUEL,1) + "  OX " + ROUND(SHIP:OXIDIZER,1) AT (0, drainLine).
        SET drainLine TO drainLine + 1.
        SET currentLine TO drainLine.
        
        // If drain was aborted, offer to continue draining or abort
        IF drainAborted {
            PRINT "" AT (0, currentLine).
            SET currentLine TO currentLine + 1.
            PRINT "SIVB drain incomplete. Press 'D' to detank all, any other key to continue" AT (0, currentLine).
            SET currentLine TO currentLine + 1.
            LOCAL drainAbortChoice IS TERMINAL:INPUT:GETCHAR().
            IF drainAbortChoice = "D" OR drainAbortChoice = "d" {
                FullDetank(allValveDrains, ALL_DRAIN_CMDS, lfCap, oxCap, monoCap).
                PRINT "Aborting launch preparation..." AT (0, currentLine).
                WAIT 5.
                REBOOT.
            }
        }
    } ELSE {
        // If no SIVB drain happens, we're still at the fueling screen
