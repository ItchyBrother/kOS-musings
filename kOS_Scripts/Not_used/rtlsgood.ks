...
LOCAL LANDING_EXIT_CLR      IS 220.   // normal early terminal handoff when close/slow enough
LOCAL LANDING_EXIT_VS       IS 30.    // allow terminal velocity-kill mode before the final seconds
...
LOCAL TOUCH_TARGET_VS     IS -5.    // normal terminal descent
LOCAL TOUCH_TARGET_VS_LOW IS -2.5.  // slow settle near ground / while killing horizontal speed
LOCAL TOUCH_LOW_CLR       IS 20.    // very low threshold (m above pad)
...
LOCAL TOUCH_KILL_CLR      IS 160.   // if close to pad below this, prioritize killing HS
...
UNTIL FALSE {
    ...
    ELSE IF phase = PH_LANDING_BURN {
        ...
        ELSE {
            ...
            // velocity matters more than closing the last few meters.
            IF clr < TOUCH_KILL_CLR AND navMiss < TOUCH_KILL_MISS AND navHs > TOUCH_KILL_HS {
                SET reqAN TO Clamp(-navVelN * 4.0 + navErrN * 0.08, -12, 12).
                SET reqAE TO Clamp(-navVelE * 4.0 + navErrE * 0.08, -12, 12).
            }
    ...
    ELSE IF phase = PH_TOUCHDOWN {
        ...
        IF clr < TOUCH_LOW_CLR OR killHoriz { SET targetVs TO TOUCH_TARGET_VS_LOW. }
        IF clr < TOUCH_FINAL_CLR { SET targetVs TO MAX(-1.0, TOUCH_TARGET_VS_LOW). }
        ...
        IF killHoriz {
            SET posGainTouch TO 0.08.
            SET posCapTouch  TO 2.0.
            SET velGainTouch TO 4.0.
            SET velCapTouch  TO 24.0.
        }
