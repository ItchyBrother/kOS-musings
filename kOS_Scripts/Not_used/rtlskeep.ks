...
LOCAL BB_TOWARD_MAX      IS 155.     // hard ceiling; avoid huge overfly past LZ
LOCAL BB_TOWARD_MARGIN   IS 0.       // no intentional over-boost; entry can fix small undershoot
LOCAL BB_AH_FRAC         IS 0.70.    // conservative fraction of thrust*sin(lean)
LOCAL BB_OVERSPEED_CUT   IS 4.       // stop boostback if padward speed exceeds target by this much
...
LOCAL LANDING_IGNITE_MARGIN IS 10.    // small hoverslam margin; avoid long high hover
LOCAL LANDING_SAFETY_FRAC   IS 0.96.  // use most available thrust; later hoverslam ignition
...
LOCAL LANDING_EXIT_CLR      IS 35.    // stay in hoverslam burn until the final meters
LOCAL LANDING_EXIT_VS       IS 35.    // allow final touchdown mode without forcing a long hover
LOCAL LANDING_LEAN_CAP_DEG  IS 25.    // hard cap on lean during landing
LOCAL LANDING_TGO_MIN       IS 2.
LOCAL LANDING_TGO_MAX       IS 15.
LOCAL LANDING_TARGET_FAST   IS 65.    // desired down-speed high in landing burn
LOCAL LANDING_TARGET_MID    IS 40.    // desired down-speed through mid-altitude
LOCAL LANDING_TARGET_LOW    IS 18.    // desired down-speed near final approach
LOCAL LANDING_TARGET_FINAL  IS 7.     // desired down-speed near the pad
...
LOCAL TOUCH_TARGET_VS     IS -8.    // terminal descent; avoid fuel-heavy hover
LOCAL TOUCH_TARGET_VS_LOW IS -4.    // short final flare, still a hoverslam
LOCAL TOUCH_LOW_CLR       IS 8.     // only flare in the last few meters
LOCAL TOUCH_VS_GAIN       IS 0.8.   // P gain on VS error
LOCAL TOUCH_POS_K         IS 0.22.  // horizontal position pull
LOCAL TOUCH_POS_CAP       IS 6.     // cap on position term; avoid late pad-chasing tipover
LOCAL TOUCH_VEL_K         IS 3.00.  // stronger horizontal velocity damping
LOCAL TOUCH_VEL_CAP       IS 18.    // cap on velocity damping term
LOCAL TOUCH_MAX_LEAN      IS 8.     // terminal velocity-kill lean cap above very low altitude
LOCAL TOUCH_MAX_LEAN_LOW  IS 3.     // tighter lean cap near ground
LOCAL TOUCH_KILL_CLR      IS 140.   // if close to pad below this, prioritize killing HS
LOCAL TOUCH_KILL_MISS     IS 90.    // close enough that velocity kill beats position chase
LOCAL TOUCH_KILL_HS       IS 1.2.   // desired horizontal speed before touchdown
LOCAL TOUCH_FREEZE_CLR    IS 75.    // once centered below this, stop pad-chasing
LOCAL TOUCH_FREEZE_MISS   IS 45.    // centered enough for terminal upright mode
LOCAL TOUCH_FREEZE_HS     IS 2.2.   // if already slow, command upright only
LOCAL TOUCH_LOCKOUT_CLR   IS 55.    // below this: no lateral pad-chasing when near pad
LOCAL TOUCH_LOCKOUT_MISS  IS 120.   // wide enough to prevent last-second divert/tipover
LOCAL TOUCH_NEAR_LEAN     IS 2.5.   // max lean when centered/low; prevents late tipover
LOCAL TOUCH_FINAL_CLR     IS 7.     // almost-on-ground lean clamp
LOCAL TOUCH_FINAL_LEAN    IS 1.2.   // do not tip over during leg contact
...
LOCAL touchFreezeLatched IS FALSE.
...
FUNCTION landingTargetDown {
    PARAMETER clrNow.
    IF clrNow > 2500 { RETURN LANDING_TARGET_FAST. }
    IF clrNow > 900  { RETURN LANDING_TARGET_MID. }
    IF clrNow > 180  { RETURN LANDING_TARGET_LOW. }
    RETURN LANDING_TARGET_FINAL.
}
...
// ------------------------------------------------------------
FUNCTION updateNav {
    LOCAL relVec  IS VXCL(upAxis(), padGeo:ALTITUDEPOSITION(padAlt)).
...
UNTIL FALSE {
    ...
    ELSE IF phase = PH_BOOSTBACK {
        ...
        ELSE {
            ...
            // FINE is allowed to trim only while still below target.
            // Do not keep adding padward velocity just to reduce crossrange;
            // CORRECTIVE is responsible for that after boostback cutoff.
            LOCAL bbGap IS bbTargetToward - navToward.
            IF bbGap <= BB_TOWARD_BAND {
                SET thrCmd TO 0.0.
            } ELSE IF bbGap <= 12 {
                SET thrCmd TO 0.03.
            } ELSE IF ABS(velErr) > 4 OR crossAbs > 4 {
                SET thrCmd TO 0.04.
            } ELSE {
                SET thrCmd TO 0.06.
            }
        ...
        // Hard boostback overspeed guard: once we have enough padward
        // velocity, cut engines even if crossrange is not perfect.
        IF bbMode >= 1 AND navToward >= bbTargetToward + BB_OVERSPEED_CUT {
            SET thrCmd TO 0.0.
        }
        ...
        IF bbMode >= 1
           AND navToward >= bbTargetToward + BB_OVERSPEED_CUT {
            SET bbExitReason TO "BOOSTBACK OVERSPEED CUT mode:" + modeName + " toward:" + ROUND(navToward,0) + " tgt:" + ROUND(bbTargetToward,0) + " cross:" + ROUND(navCross,1) + " velErr:" + ROUND(velErr,1).
        }
        ELSE IF bbMode >= 1
           AND thrCmd <= 0.001
    ...
    ELSE IF phase = PH_LANDING_BURN {
        ...
        IF NOT landingIgnited {
            ...
            // SHIP:AVAILABLETHRUST reflects the current engine set capability.
            LOCAL tmMax IS SHIP:AVAILABLETHRUST / MAX(0.1, SHIP:MASS).
            LOCAL aNet  IS MAX(1, tmMax * LANDING_SAFETY_FRAC - padG).

            // Kinematic stopping distance to a nonzero hoverslam descent
            // rate. Targeting zero here starts a long hover-burn too high.
            LOCAL tgtDownIgn IS landingTargetDown(clr).
            LOCAL stopDist IS MAX(0, (vsDown * vsDown - tgtDownIgn * tgtDownIgn) / (2 * aNet)).
        ...
        ELSE {
            ...
            // Required vertical deceleration toward a nonzero target
            // down-speed. This keeps the landing burn a hoverslam instead of
            // forcing a zero-VS lunar-style hover profile.
            LOCAL clrFloor IS MAX(20, clr).
            LOCAL targetDownLand IS landingTargetDown(clrFloor).
            LOCAL aVertReq IS ((vsDown * vsDown) - (targetDownLand * targetDownLand)) / (2 * clrFloor) + padG.
            ...
            // Terminal horizontal-kill override.  When nearly over the pad,
            // velocity matters more than closing the last few meters.  If already
            // centered and slow below the freeze gate, stay upright instead of
            // making one last lateral correction that can tip the booster over.
            IF clr < TOUCH_LOCKOUT_CLR AND navMiss < TOUCH_LOCKOUT_MISS {
                // We are close enough and too low for lateral steering to help.
                // Do not chase the last meters; stay upright and protect the legs.
                SET reqAN TO 0.
                SET reqAE TO 0.
            } ELSE IF clr < TOUCH_FREEZE_CLR AND navMiss < TOUCH_FREEZE_MISS AND navHs < TOUCH_FREEZE_HS {
                SET reqAN TO 0.
                SET reqAE TO 0.
            } ELSE IF clr < TOUCH_KILL_CLR AND navMiss < TOUCH_KILL_MISS AND navHs > TOUCH_KILL_HS {
                LOCAL landVelGain IS 2.0.
                LOCAL landVelCap  IS 6.0.
                IF clr < TOUCH_FREEZE_CLR AND navMiss < TOUCH_FREEZE_MISS {
                    SET landVelGain TO 1.2.
                    SET landVelCap  TO 3.0.
                }
                SET reqAN TO Clamp(-navVelN * landVelGain + navErrN * 0.02, -landVelCap, landVelCap).
                SET reqAE TO Clamp(-navVelE * landVelGain + navErrE * 0.02, -landVelCap, landVelCap).
            }
            ...
            // Exit to TOUCHDOWN when low/slow. If we are already close to
            // the pad, enter terminal mode earlier so it can kill horizontal
            // velocity before the legs touch.
            LOCAL terminalReady IS FALSE.
            IF clr <= LANDING_EXIT_CLR AND vsDown <= LANDING_EXIT_VS AND navMiss <= TOUCH_KILL_MISS * 1.5 {
                SET terminalReady TO TRUE.
            }
            IF clr <= 120 AND vsDown <= LANDING_EXIT_VS {
                SET terminalReady TO TRUE.
            }

            IF terminalReady {
                SET touchStartTime TO TIME:SECONDS.
                SET touchFreezeLatched TO FALSE.
    ...
    ELSE IF phase = PH_TOUCHDOWN {
        ...
        // Target descent rate. When we are near the pad but still sliding,
        // spend the last ~100 m killing HS instead of carrying lateral
        // motion into the legs.
        LOCAL killHoriz IS FALSE.
        LOCAL freezeTerminal IS FALSE.

        // Sticky terminal lockout: once we are low and near the pad, never
        // re-enable lateral pad-chasing. Previous builds could be centered
        // at ~40 m, then re-enter kill mode after a tiny speed change and
        // shove the booster off the bullseye.
        IF touchFreezeLatched {
            SET freezeTerminal TO TRUE.
        } ELSE IF clr < TOUCH_LOCKOUT_CLR AND navMiss < TOUCH_LOCKOUT_MISS {
            SET freezeTerminal TO TRUE.
            SET touchFreezeLatched TO TRUE.
        } ELSE IF clr < TOUCH_FREEZE_CLR AND navMiss < TOUCH_FREEZE_MISS AND navHs < TOUCH_FREEZE_HS {
            SET freezeTerminal TO TRUE.
            SET touchFreezeLatched TO TRUE.
        }

        IF NOT freezeTerminal AND clr < TOUCH_KILL_CLR AND navMiss < TOUCH_KILL_MISS AND navHs > TOUCH_KILL_HS {
            SET killHoriz TO TRUE.
        }

        LOCAL targetVs IS TOUCH_TARGET_VS.
        IF clr < TOUCH_LOW_CLR { SET targetVs TO TOUCH_TARGET_VS_LOW. }
        IF freezeTerminal { SET targetVs TO TOUCH_TARGET_VS. }
        IF clr < TOUCH_FINAL_CLR { SET targetVs TO TOUCH_TARGET_VS_LOW. }
        ...
        // Horizontal correction. Close to the pad, velocity kill dominates;
        // position pull is deliberately weak so we do not chase the pad and
        // tip over after already crossing the bullseye.
        LOCAL posGainTouch IS TOUCH_POS_K.
        LOCAL posCapTouch  IS TOUCH_POS_CAP.
        LOCAL velGainTouch IS TOUCH_VEL_K.
        LOCAL velCapTouch  IS TOUCH_VEL_CAP.
        IF killHoriz {
            SET posGainTouch TO 0.00.
            SET posCapTouch  TO 0.0.
            SET velGainTouch TO 2.0.
            SET velCapTouch  TO 6.0.
            IF clr < TOUCH_FREEZE_CLR AND navMiss < TOUCH_FREEZE_MISS {
                SET velGainTouch TO 1.2.
                SET velCapTouch  TO 3.0.
            }
        }
        IF freezeTerminal {
            SET posGainTouch TO 0.00.
            SET posCapTouch  TO 0.0.
            SET velGainTouch TO 0.0.
            SET velCapTouch  TO 0.0.
        }

        LOCAL corrN IS Clamp(-navVelN * velGainTouch, -velCapTouch, velCapTouch)
                     + Clamp( navErrN * posGainTouch, -posCapTouch, posCapTouch).
        LOCAL corrE IS Clamp(-navVelE * velGainTouch, -velCapTouch, velCapTouch)
                     + Clamp( navErrE * posGainTouch, -posCapTouch, posCapTouch).

        // Lean cap: enough authority above the legs to kill 3-6 m/s HS,
        // then clamp hard in the last few meters to avoid tip-over.
        LOCAL leanCap IS TOUCH_MAX_LEAN.
        IF clr < TOUCH_FREEZE_CLR AND navMiss < TOUCH_FREEZE_MISS { SET leanCap TO TOUCH_NEAR_LEAN. }
        IF freezeTerminal { SET leanCap TO 0. }
        IF clr < 40 { SET leanCap TO MIN(leanCap, TOUCH_MAX_LEAN_LOW). }
        IF clr < TOUCH_FINAL_CLR { SET leanCap TO TOUCH_FINAL_LEAN. }

        LOCAL corrMag IS SQRT(corrN * corrN + corrE * corrE).
        LOCAL leanTouch IS 0.
        IF corrMag > 0.1 {
            SET leanTouch TO MIN(leanCap, MAX(0.5, corrMag * 0.45)).
        }
        ...
        IF freezeTerminal OR clr < TOUCH_LOCKOUT_CLR {
            RCS OFF.
        } ELSE {
            RCS ON.
        }
        ...
        showStatus(
            "TOUCHDOWN",
            "thr:" + ROUND(thrTouch*100,0) + "%  tgtVs:" + ROUND(targetVs,1) + "  lean:" + ROUND(leanTouch,1) + "  kill:" + killHoriz,
