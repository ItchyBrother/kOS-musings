// ============================================================
// rtls1.ks - azimuth-independent RTLS accuracy + terminal stability patch
//
...
//
// rtls1 azimuth patch:
//   * Uses GeoCoordinates:ALTITUDEPOSITION directly as the pad vector
//     and projects it onto the horizontal plane.
//   * Removes east-axis-only BOOSTBACK gates and fine-mode bias.
//   * Adds a cached 2mF-style predicted-impact estimate.
//   * Allows BOOSTBACK handoff from projected impact, and uses projected
//     impact error for AERO correction.
//
// rtls1 terminal-stability patch after reported 790m southwest miss:
//   * Default pad updated to -0.185464 / -74.472935.
//   * AERO/pre-ignition correction is stronger and stays active until ignition.
//   * LANDING/TOUCHDOWN now taper horizontal pad-chase near the ground, so a
//     late large miss does not command a tip-over trying to reach the pad.
//   * Display is formatted for a 50x36 kOS terminal.
//
// ============================================================

@LAZYGLOBAL OFF.

PARAMETER padLatP IS -0.185464.
PARAMETER padLngP IS -74.472935.
...
LOCAL AERO_LEAN_DEG      IS 11.      // stronger high-altitude aero correction; terminal phases taper later
LOCAL AERO_POS_K         IS 0.024.   // legacy position gain fallback
LOCAL AERO_POS_CAP       IS 14.      // legacy position cap fallback
LOCAL AERO_IMPACT_K      IS 0.026.   // proactive predicted-impact correction
LOCAL AERO_IMPACT_CAP    IS 18.      // cap on predicted-impact aero nudge
LOCAL IMPACT_K           IS 5.5.     // 2mF-style drag/impact predictor coefficient
LOCAL IMPACT_DT          IS 1.0.     // predictor timestep, seconds
LOCAL IMPACT_TMAX        IS 260.     // max predictor time horizon, seconds
LOCAL IMPACT_UPDATE_DT   IS 0.4.     // cache interval; keeps kOS CPU load reasonable
LOCAL BB_IMPACT_ANGLE_OK IS 12.      // boostback solved when projected impact is near pad azimuth
LOCAL BB_IMPACT_OVER_OK  IS 300.     // ...and projected impact overshoots pad by this many meters
...
LOCAL LANDING_LEAN_CAP_DEG  IS 18.    // upper landing lean cap; reduced to avoid late tip-over
LOCAL LANDING_TGO_MIN       IS 2.
LOCAL LANDING_TGO_MAX       IS 15.
LOCAL LANDING_POS_TAPER_HIGH IS 1500. // below this, fade out pad-chase position term
LOCAL LANDING_POS_TAPER_LOW  IS 250.  // by this clearance, mostly kill lateral velocity
LOCAL LANDING_HOLD_CLR       IS 180.  // if still far this low, stop chasing pad
LOCAL LANDING_HOLD_MISS      IS 150.  // miss threshold for late upright hold
LOCAL LANDING_LOW_LEAN_CLR   IS 500.  // taper lean cap below this clearance
LOCAL LANDING_LOW_LEAN_CAP   IS 7.
LOCAL LANDING_FINAL_LEAN_CLR IS 120.
LOCAL LANDING_FINAL_LEAN_CAP IS 2.5.
...
LOCAL TOUCH_POS_TAPER_CLR IS 100.   // fade pad-chase to zero near touchdown
LOCAL TOUCH_VS_GAIN       IS 0.8.   // P gain on VS error
LOCAL TOUCH_POS_K         IS 0.08.  // gentle final position pull; big misses are handled earlier
LOCAL TOUCH_POS_CAP       IS 2.5.   // final pad-chase cap to prevent touchdown tip-over
LOCAL TOUCH_VEL_K         IS 1.10.  // horizontal velocity damping
LOCAL TOUCH_VEL_CAP       IS 5.     // cap on final damping term
LOCAL TOUCH_MAX_LEAN      IS 2.5.   // max lean during touchdown
LOCAL TOUCH_MAX_LEAN_LOW  IS 0.8.   // nearly upright at ground contact
...
// Cached predicted-impact state - refreshed by updateImpactPrediction()
LOCAL impactErrN         IS 0.
LOCAL impactErrE         IS 0.
LOCAL impactMiss         IS 0.
LOCAL impactRange        IS 0.
LOCAL impactPadRange     IS 0.
LOCAL impactAngle        IS 180.
LOCAL impactLastUpdate   IS -999.
...
FUNCTION sayAt {
    ...
    // 50-column terminal line clear. Keep callers short to avoid wrap.
    PRINT msg + "                                                  " AT (0, rowNum).
}

FUNCTION showStatus {
    PARAMETER title, detail1 IS "", detail2 IS "".
    sayAt(0, "RTLS1 " + title + "  T+" + ROUND(TIME:SECONDS,0)).
    sayAt(1, "Pad " + ROUND(navMiss,0) + "m  " + nsText(navErrN) + " " + ewText(navErrE)).
    sayAt(2, "Vel H:" + ROUND(navHs,1) + "  V:" + ROUND(SHIP:VERTICALSPEED,1)).
    sayAt(3, "Clr:" + ROUND(navClearance,0) + "  Alt:" + ROUND(SHIP:ALTITUDE,0) + "  Best:" + ROUND(bestMissSeen,0)).
    sayAt(4, "Imp:" + ROUND(impactMiss,0) + "  dR:" + ROUND(impactRange-impactPadRange,0) + "  Ang:" + ROUND(impactAngle,0)).
    sayAt(5, detail1).
    sayAt(6, detail2).
    sayAt(7, "                                                  ").
}
...
// ------------------------------------------------------------
FUNCTION updateNav {
    // GeoCoordinates:ALTITUDEPOSITION is already a ship-raw vector
    // from the vessel to the requested pad point. Project it onto
    // the local horizontal plane so all later guidance is pad-vector
    // based instead of east-axis based.
    LOCAL padVec  IS padGeo:ALTITUDEPOSITION(padAlt).
    LOCAL relVec  IS VXCL(upAxis(), padVec).
    LOCAL hVelVec IS VXCL(upAxis(), SHIP:VELOCITY:SURFACE).

    SET navErrN   TO VDOT(relVec, northAxis()).
    SET navErrE   TO VDOT(relVec, eastAxis()).
    SET navMiss   TO relVec:MAG.
    SET navVelN   TO VDOT(hVelVec, northAxis()).
    SET navVelE   TO VDOT(hVelVec, eastAxis()).
    SET navHs     TO hVelVec:MAG.
...
// ------------------------------------------------------------
// 2mF-style predicted impact guidance helper
// ------------------------------------------------------------
FUNCTION predictImpactHoriz {
    PARAMETER k.
    PARAMETER dt.
    PARAMETER tMax.
    ...
    LOCAL t  IS 0.
    LOCAL groundDrop IS MAX(1, terrainAGLCalc()).

    UNTIL VDOT(ps, upAxis()) < -groundDrop OR t > tMax {
        LOCAL altSim IS SHIP:ALTITUDE + VDOT(ps, upAxis()).
        IF altSim < 0 { SET altSim TO 0. }

        LOCAL dragA IS V(0,0,0).
        IF vs:MAG > 0.1 {
            SET dragA TO -vs:NORMALIZED * (vs:MAG ^ 2 * k * (0.99997 ^ altSim)) / (MAX(0.1, SHIP:MASS) * 1000).
        }

        LOCAL gravA IS -upAxis() * padG.
        SET vs TO vs + (dragA + gravA) * dt.
        SET ps TO ps + vs * dt.
        SET t TO t + dt.
    }

    RETURN VXCL(upAxis(), ps).
}

FUNCTION updateImpactPrediction {
    // Cache this relatively expensive ballistic/aero prediction so
    // the main loop remains smooth. Existing cached values remain
    // available between updates.
    IF TIME:SECONDS - impactLastUpdate < IMPACT_UPDATE_DT {
        RETURN FALSE.
    }
    SET impactLastUpdate TO TIME:SECONDS.

    LOCAL padH IS VXCL(upAxis(), padGeo:ALTITUDEPOSITION(padAlt)).
    LOCAL impH IS predictImpactHoriz(IMPACT_K, IMPACT_DT, IMPACT_TMAX).
    LOCAL errH IS padH - impH.

    SET impactPadRange TO padH:MAG.
    SET impactRange    TO impH:MAG.
    SET impactMiss     TO errH:MAG.
    SET impactErrN     TO VDOT(errH, northAxis()).
    SET impactErrE     TO VDOT(errH, eastAxis()).

    IF padH:MAG > 1 AND impH:MAG > 1 {
        SET impactAngle TO VANG(padH, impH).
    } ELSE {
        SET impactAngle TO 180.
    }
...
CLEARSCREEN.
//logOpen().
PRINT "Loaded rtls1 terminal-stability patch".
updateNav().
...
UNTIL FALSE {
    ...
    ELSE IF phase = PH_FLIP {
        ...
        showStatus("FLIP", "pitch:" + ROUND(flipErr,1) + " brg:" + ROUND(navPadBrg,1), "BB tgt:" + ROUND(bbTargetToward,0) + "m/s").
    ...
    ELSE IF phase = PH_BOOSTBACK {
        ...
        LOCAL impactSolved IS FALSE.

        IF bbMode = 0 AND navToward >= BB_REV_FLOOR {
            SET bbMode TO 1.
            SET bbAimBrg TO lineBrg.
        }
        IF bbMode = 1 AND navToward >= bbTargetToward - 8 AND crossAbs <= 8 AND ABS(velErr) <= 10 {
            SET bbMode TO 2.
        }
        IF bbMode = 2 AND (crossAbs > 12 OR ABS(velErr) > 12) {
            SET bbMode TO 1.
        }
        ...
        ELSE {
            ...
            SET headBias TO Clamp((-navCross * 0.10) + Clamp((-velErr * 0.05), -2, 2), -4, 4).
            SET desiredAim TO wrapBrg(lineBrg + headBias).
        ...
        IF bbMode >= 1
           AND impactAngle < BB_IMPACT_ANGLE_OK
           AND impactRange > impactPadRange + BB_IMPACT_OVER_OK
           AND crossAbs < 15 {
            SET impactSolved TO TRUE.
            SET thrCmd TO 0.0.
        }
        ...
        showStatus("BOOSTBACK", "" + modeName + " aim:" + ROUND(aimBrg,1) + " thr:" + ROUND(thrCmd*100,0) + "%", "tow:" + ROUND(navToward,0) + " tgt:" + ROUND(bbTargetToward,0) + " x:" + ROUND(navCross,0) + " vE:" + ROUND(velErr,1)).
        ...
        IF bbMode >= 1
           AND thrCmd <= 0.001
           AND (impactSolved OR navToward >= bbTargetToward - BB_TOWARD_BAND) {
            IF impactSolved {
                SET bbExitReason TO "predicted-impact handoff mode:" + modeName + " impMiss:" + ROUND(impactMiss,0) + " impAng:" + ROUND(impactAngle,1) + " rangeOver:" + ROUND(impactRange-impactPadRange,0) + " cross:" + ROUND(navCross,1).
            } ELSE {
    ...
    ELSE IF phase = PH_CORRECTIVE {
        ...
        showStatus("CORRECTIVE", corrName + " aim:" + ROUND(aimBrgC,1) + " thr:" + ROUND(thrCorr*100,0) + "%", "tow:" + ROUND(navToward,0) + " x:" + ROUND(navCross,0) + " vE:" + ROUND(velErrC,1)).
    ...
    ELSE IF phase = PH_COAST {
        ...
        showStatus("COAST", "engine-first trim", "fins:" + coastFinsOut + " apo:" + coastApoSeen + " brg:" + ROUND(navPadBrg,1)).
    ...
    ELSE IF phase = PH_ENTRY {
        ...
        showStatus(
            "ENTRY",
            "tgo:" + ROUND(tgoEntry,1) + " lean:" + ROUND(leanEff,1) + " thr:" + ROUND(thrEntry*100,0) + "%",
            "aN/E:" + ROUND(reqAN,1) + "/" + ROUND(reqAE,1) + " el:" + ROUND(entryElapsed,1)
        ).
        // logPeriodic(
    ...
    ELSE IF phase = PH_AERO {
        ...
        // Predicted-impact pull. During aero, current position error
        // can be misleading; this steers from where the booster is
        // projected to hit toward where the pad actually is. Sign
        // convention remains inverted for fin reaction: -impactErr
        // tilts away from the needed correction so aero force pulls
        // toward it.
        IF impactMiss > 30 {
            LOCAL nudgeN IS Clamp(-impactErrN * AERO_IMPACT_K, -AERO_IMPACT_CAP, AERO_IMPACT_CAP).
            LOCAL nudgeE IS Clamp(-impactErrE * AERO_IMPACT_K, -AERO_IMPACT_CAP, AERO_IMPACT_CAP).
            SET corrN_A TO corrN_A + nudgeN.
            SET corrE_A TO corrE_A + nudgeE.
        } ELSE IF navMiss > 40 {
            // Fallback if the predictor is stale or already very close.
        ...
        showStatus(
            "AERO",
            "impact pull lean:" + ROUND(AERO_LEAN_DEG,1),
            "best:" + ROUND(bestMissSeen,0) + "@" + ROUND(bestMissAlt,0) + " hs:" + ROUND(navHs,1)
        ).
        // logPeriodic(
    ...
    ELSE IF phase = PH_LANDING_BURN {
        ...
        IF NOT landingIgnited {
            ...
            // but keep the impact-point pull active until ignition. The prior
            // version dropped the impact pull during this coast, allowing the
            // miss to grow before the engines lit.
            LOCAL corrN_pre IS navVelN.
            LOCAL corrE_pre IS navVelE.
            IF impactMiss > 30 {
                LOCAL nudgeN_pre IS Clamp(-impactErrN * AERO_IMPACT_K, -AERO_IMPACT_CAP, AERO_IMPACT_CAP).
                LOCAL nudgeE_pre IS Clamp(-impactErrE * AERO_IMPACT_K, -AERO_IMPACT_CAP, AERO_IMPACT_CAP).
                SET corrN_pre TO corrN_pre + nudgeN_pre.
                SET corrE_pre TO corrE_pre + nudgeE_pre.
            }
            ...
            showStatus(
                "LANDING",
                "pre ign stop:" + ROUND(stopDist,0) + " ign:" + ROUND(ignAlt,0),
                "clr:" + ROUND(clr,0) + " vs:" + ROUND(vsLand,1) + " hs:" + ROUND(navHs,1)
            ).
            // logPeriodic(
        ...
        ELSE {
            ...
            // ZEM/ZEV horizontal acceleration request. Fade the position
            // term out near the ground. If the booster is still far off-pad
            // very low, prioritize upright landing and lateral velocity kill
            // instead of making a last-second sideways lunge.
            LOCAL posScale IS 1.
            IF clr < LANDING_POS_TAPER_HIGH {
                SET posScale TO Clamp((clr - LANDING_POS_TAPER_LOW) / (LANDING_POS_TAPER_HIGH - LANDING_POS_TAPER_LOW), 0, 1).
            }
            IF clr < LANDING_HOLD_CLR AND navMiss > LANDING_HOLD_MISS {
                SET posScale TO 0.
            }

            // Dynamic lean cap: enough authority up high, very little at touchdown.
            LOCAL leanCapLand IS LANDING_LEAN_CAP_DEG.
            IF clr < LANDING_LOW_LEAN_CLR { SET leanCapLand TO LANDING_LOW_LEAN_CAP. }
            IF clr < LANDING_FINAL_LEAN_CLR { SET leanCapLand TO LANDING_FINAL_LEAN_CAP. }

            LOCAL tgo2 IS tgoLand * tgoLand.
            LOCAL reqAN IS (6 / tgo2) * navErrN * posScale - (8 / tgoLand) * navVelN.
            LOCAL reqAE IS (6 / tgo2) * navErrE * posScale - (8 / tgoLand) * navVelE.
            ...
            // Also cap by the terminal lean schedule BEFORE throttle is computed;
            // this keeps vertical accel honest when lean is limited.
            LOCAL aHleanMax IS aVertReq * TAN(leanCapLand).
            IF reqAHmag > aHleanMax AND reqAHmag > 0.01 {
                LOCAL scaleLean IS aHleanMax / reqAHmag.
                SET reqAN TO reqAN * scaleLean.
                SET reqAE TO reqAE * scaleLean.
                SET reqAHmag TO aHleanMax.
            }
            ...
            IF leanLand > leanCapLand { SET leanLand TO leanCapLand. }
            ...
            showStatus(
                "LANDING",
                "thr:" + ROUND(thrLand*100,0) + "% lean:" + ROUND(leanLand,1) + " tgo:" + ROUND(tgoLand,1),
                "pS:" + ROUND(posScale,2) + " cap:" + ROUND(leanCapLand,1) + " solo:" + landingSoloMode
            ).
            // logPeriodic(
    ...
    ELSE IF phase = PH_TOUCHDOWN {
        ...
        // Horizontal correction: mostly damp velocity. Fade pad-chase
        // position pull near touchdown so a remaining large miss cannot tip
        // the booster over in the final seconds.
        LOCAL touchPosScale IS Clamp(clr / TOUCH_POS_TAPER_CLR, 0, 1).
        IF clr < TOUCH_LOW_CLR { SET touchPosScale TO 0. }
        IF clr < 120 AND navMiss > 150 { SET touchPosScale TO 0. }

        LOCAL corrN IS Clamp(-navVelN * TOUCH_VEL_K, -TOUCH_VEL_CAP, TOUCH_VEL_CAP)
                     + Clamp( navErrN * TOUCH_POS_K * touchPosScale, -TOUCH_POS_CAP, TOUCH_POS_CAP).
        LOCAL corrE IS Clamp(-navVelE * TOUCH_VEL_K, -TOUCH_VEL_CAP, TOUCH_VEL_CAP)
                     + Clamp( navErrE * TOUCH_POS_K * touchPosScale, -TOUCH_POS_CAP, TOUCH_POS_CAP).
        ...
        showStatus(
            "TOUCHDOWN",
            "thr:" + ROUND(thrTouch*100,0) + "% tgtV:" + ROUND(targetVs,1) + " lean:" + ROUND(leanTouch,1),
            "clr:" + ROUND(clr,1) + " pS:" + ROUND(touchPosScale,2)
        ).
        // logPeriodic(
    ...
    ELSE IF phase = PH_IMPACT {
        ...
        showStatus(
            "IMPACT",
            "shutdown  status:" + SHIP:STATUS,
            "miss:" + ROUND(navMiss,0) + " best:" + ROUND(bestMissSeen,0) + " hs:" + ROUND(navHs,1)
        ).
        BREAK.
    }
