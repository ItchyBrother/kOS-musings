...
// SSME realism  Uses maxq_throt as minimum thrust level.
// Enter peMode when Ap is within this many km of target (dynamic, orbit-agnostic)
GLOBAL apMarginEnterKm IS 25.     // tune 15..35

// Pitch balancing gains for peMode
GLOBAL ap_pitch_kp IS 0.35.       // deg per km of Ap overshoot (tune 0.25..0.55)
GLOBAL pe_pitch_kp IS 0.03.       // deg per km of Pe shortfall (tune 0.01..0.04)
...
// Dynamic targets based on menu values
LOCAL peGoal IS MIN(pe_goal, target_Pe).

// Scale Ap-hold band for higher orbits (keeps behavior consistent)
LOCAL apHoldBandNow IS MAX(ap_hold_band, target_Ap * 0.005).  // 0.5% of target_Ap, min ap_hold_band

// Dynamic locals (computed once)
LOCAL apMarginEnter IS apMarginEnterKm * 1000.
LOCAL peEnableAlt IS turn_end_alt.                   // use your existing value


// Main ascent loop
//UNTIL SHIP:APOAPSIS >= target_Ap {
//UNTIL (SHIP:ORBIT:APOAPSIS >= target_Ap AND SHIP:ORBIT:PERIAPSIS >= peGoal) {
UNTIL FALSE {

    // Turn on peMode only when we're out of thick air and close to target Ap
    IF (NOT peMode) AND (SHIP:ALTITUDE >= turn_end_alt) AND (SHIP:ORBIT:APOAPSIS >= (target_Ap - apMarginEnter)) {
        SET peMode TO TRUE.
        SET apHoldThrottle TO smoothedThrottle.
    ...
    // >>> PLACE THE SSME MIN THROTTLE CLAMP RIGHT HERE <<<
    IF MODE = 2 {
        SET throttleCmd TO Clamp(throttleCmd, maxq_throt, 1.0).  // 0.65..1.0
    }

    // Slew / smooth (your existing code)
    ...
    IF ascentGuidanceActive {
        ...
        IF flRoll AND NOT Rolled AND SHIP:ALTITUDE > 50000 {
        ...
        // Base pitch from your schedule (normal ascent)
        LOCAL pitchCmd IS TargetPitch().

        IF peMode {
                // errors in km
                LOCAL apErrKm IS (SHIP:ORBIT:APOAPSIS - target_Ap) / 1000.    // + = Ap high
                LOCAL peErrKm IS (peGoal - SHIP:ORBIT:PERIAPSIS) / 1000.      // + = Pe below goal

                // Start from your pe_pitch_cmd (flat), flatten MORE if Pe is very negative:
                // (positive peErr -> subtract -> smaller pitch -> more horizontal speed)
                LOCAL peBias IS Clamp(peErrKm * pe_pitch_kp, 0, 12).          // 0..12 deg flattening
                SET pitchCmd TO pe_pitch_cmd - peBias.

                    IF apErrKm > 0 {
                        SET pitchCmd TO pitchCmd - Clamp(apErrKm * ap_pitch_kp, 0, 15).
                    }

                LOCAL minPitchNow IS pe_pitch_min.
                IF flRoll AND Rolled { SET minPitchNow TO -5. }

                SET pitchCmd TO Clamp(pitchCmd, minPitchNow, pe_pitch_max).

                // If Ap is above target, pitch UP to stop Ap ballooning (since throttle can't go < 65%)
                LOCAL apBias IS 0.
                IF apErrKm > 0 {
                    SET apBias TO Clamp(apErrKm * ap_pitch_kp, 0, 15).          // 0..15 deg pitch-up
                }

                SET pitchCmd TO pe_pitch_cmd - peBias + apBias.

                // Heads-up safety (optional)
                LOCAL minPitchNow IS pe_pitch_min.
                IF flRoll AND Rolled { SET minPitchNow TO 0. }
        ...
        PRINT "PitchCmd:    " + ROUND(pitchCmd,1) + "     " AT (0, 9).
    ...
    // }
        ...
        // Safety: prevent runaway Ap
    IF SHIP:ORBIT:APOAPSIS >= (target_Ap + 10000) {
        PRINT "SAFETY: Ap runaway -> MECO" AT (0, 17).
        BREAK.
    }


    // Exit condition: we’ve met both targets on the same continuous ET burn
    IF peMode AND (SHIP:ORBIT:APOAPSIS >= target_Ap) AND (SHIP:ORBIT:PERIAPSIS >= peGoal) {
        PRINT "Main ascent burn complete." AT (0, 17).
        BREAK.
    }
...
LOCK THROTTLE TO 0.
WAIT 20.  //Giving us some distance from the ET.
...
// Added a few safe guards to ensure we always get above Kerbin atmophere.
// If our apoapsis is above our target Ap by over 5000 meters (5km) we target our Pe
// first to make sure it gets above the atmosphere. 
IF SHIP:PERIAPSIS < 70000 AND SHIP:APOAPSIS > 70000 {
    RUNPATH ("circ.ks", SHIP:APOAPSIS, target_Pe, orbitMode).
}

IF SHIP:APOAPSIS >= target_Ap + 5000 {
    RUNPATH ("circ.ks", SHIP:APOAPSIS, target_Pe, orbitMode). 
} ELSE {
    RUNPATH ("circ.ks", target_Ap, target_Pe, orbitMode).
}

// Since we are above Kerbin's atmosphere, we need to make sure our Apopasis is correct.

IF SHIP:PERIAPSIS >= 72000 AND SHIP:APOAPSIS > target_Ap + 5000 {
    RUNPATH ("circ.ks", target_Ap, target_Pe, orbitMode).
}

AG4 OFF. // Deactivate OMS.
// If circ returns, stop this script.
AG1 ON.  // Refocusing our control point.
WAIT 10.
RCS ON.
LOCK STEERING TO PROGRADE.
Stable().
